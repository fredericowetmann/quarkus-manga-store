...
import br.unitins.topicos2.model.CardBrand;
import br.unitins.topicos2.model.Comic;
import br.unitins.topicos2.model.CreditCard;
...
import br.unitins.topicos2.repository.CreditCardRepository;
...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderResponseDTO insert(OrderDTO dto, String login) {
        ...
        for (ItemOrderDTO itemDto : dto.itens()) {
            total += (comicRepository.findById(itemDto.idComic()).getPrice() * itemDto.quantity());
        }
        pedido.setTotalOrder(total);

        pedido.setItens(new ArrayList<ItemOrder>());
        for (ItemOrderDTO itemDto : dto.itens()) {
            if(itemDto.quantity() > comicService.findById(itemDto.idComic()).inventory()) {
                throw new ValidationException("400", "Estoque insuficiente");
            }
            ItemOrder item = new ItemOrder();
            item.setPrice(comicRepository.findById(itemDto.idComic()).getPrice());
            item.setQuantity(itemDto.quantity());
            item.setOrder(pedido);
            Comic comic = comicRepository.findById(itemDto.idComic());
            item.setComic(comic);
